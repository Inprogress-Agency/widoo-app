...
- **Tokens de design** : `packages/tokens/generated/` ne s'édite jamais à la main ; `tokens.json` y est la copie du wiki (`design/design-system/tokens.json`). L'action `sync-tokens.yml` (toutes les heures, à la demande, et à chaque push du wiki qui édite une page) ouvre ou met à jour la PR « chore(tokens): synchroniser tokens.json vX depuis le wiki », thème régénéré. En CI, un écart avec le wiki n'est qu'un avertissement, sauf pour une PR qui touche les fichiers générés (thème, `tokens.json`), la config Tailwind ou le CSS global de l'app : la synchroniser d'abord (`pnpm --filter @widoo/tokens sync`, puis `generate`).
